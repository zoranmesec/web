...
export class ScrollService {
    scrollPositions: Record<number, number> = {}; // event id -> scroll position
    ...
    startCachingScrollPositions() {
        this.router.events.pipe(filter((event) => event instanceof NavigationStart)).subscribe((event: NavigationStart) => {
            this.scrollPositions[event.id] = this.viewportScroller.getScrollPosition()[1];
    ...
    restoreScroll() {
        asyncScheduler.schedule(() => {
            ...
            if (this.lastEvent.navigationTrigger === 'popstate') {
                this.viewportScroller.scrollToPosition([0, this.scrollPositions[this.lastEvent.restoredState.navigationId + 1]]);
            }
        });
    }
}
