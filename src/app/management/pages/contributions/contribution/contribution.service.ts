...
export class ContributionService {
     ...
     */
    nextPossiblePublishStatuses(contributionEntity: Route | Sector | Crag): { enabled: boolean; status: string }[] {
        ...
        // in_review --> published
        // in_review --> draft
        if (this.user.roles.includes('admin') && contributionEntity.publishStatus === 'in_review') {
            // entity can be published only if parent entity is already published
            let publishEnabled = false;
            switch (contributionEntity.__typename) {
                case 'Route':
                    publishEnabled = contributionEntity.sector.publishStatus === 'published';
                    break;
                case 'Sector':
                    publishEnabled = contributionEntity.crag.publishStatus === 'published';
        ...
        if (
            ...
            switch (contributionEntity.__typename) {
                case 'Route':
                    enabled = ['published', 'in_review'].includes(contributionEntity.sector.publishStatus);
                    break;
                case 'Sector':
     ...
     */
    statusButtonsTrackBy = (item: { status: string; enabled: boolean }) => item.status + item.enabled;
            ...
            case 'Crag':
                ...
                if (newStatus === 'in_review' && contributionEntity.sectors.length) {
                    ...
                    if (contributionEntity.sectors.some((sector) => sector.routes.length)) {
                        cascadeMessage = 'Predlagaj tudi objavo vseh sektorjev in vseh smeri v tem plezališču.';
                    }
                }
    ...
    private displayError() {
        this.snackbar.open('Pri obdelavi zahteve je prišlo do nepričakovane napake.', null, {
            panelClass: 'error',
            duration: 3000
        });
    }
