...
export class ContributionsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.subscription = this.managementContributionsGQL.watch().valueChanges.subscribe({
            next: (response) => {
                this.contributionsTree = this.makeContributionsTree(response.data.contributions as Contribution[]);
                this.loading = false;
            },
            error: () => {
                this.loading = false;
