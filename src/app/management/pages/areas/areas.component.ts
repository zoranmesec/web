...
import { Area, Country, CountryAreasGQL, ManagementDeleteAreaGQL } from 'src/generated/graphql';
...
export class AreasComponent implements OnInit, OnDestroy {
    ...
    hasDescendants({ areas, crags, iceFalls, peaks }: Area): boolean {
        return !!areas.length || !!crags.length || !!iceFalls.length || !!peaks.length;
    }
    ...
    remove(area: Area): void {
        this.dialog
            ...
            .pipe(
                ...
                switchMap(() => this.deleteAreaGQL.mutate({ variables: { id: area.id } }))
            )
            .subscribe({
                next: () =>
