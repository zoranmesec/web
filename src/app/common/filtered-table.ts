...
export class FilteredTable {
    ...
    setRouteParams(values: Params) {
        ...
        this.filters.forEach((filter) => {
            if (filter.type === 'date' && values[filter.name] !== undefined) {
                fp[filter.name] = dayjs(values[filter.name]).format('YYYY-MM-DD');
                qp[filter.name] = values[filter.name];
                return;
            }
            if (filter.type === 'multiselect' && values[filter.name] !== undefined) {
                fp[filter.name] = values[filter.name].split(',');
                qp[filter.name] = values[filter.name].split(',');
                return;
            }
            if (filter.type === 'relation' && values[filter.name] !== undefined) {
                fp[filter.name] = values[filter.name];
                qp[filter.name] = values[filter.name];
                return;
            }
            if (filter.type === 'number' && values[filter.name] !== undefined) {
    ...
    setFilterParams(values: Params, navigate = true) {
        ...
        if (this.initialized && JSON.stringify(this.filterParams) !== JSON.stringify(fp)) {
            this.routeParams.pageNumber = 1;
        }

        this.filterParams = fp;

        if (this.routeParams.pageNumber !== null && this.routeParams.pageNumber !== 1) {
            rp.pageNumber = this.routeParams.pageNumber;
        }

        const defaultSortCol = this.columns.find((col) => col.defaultSort !== null);
        if (defaultSortCol === null || defaultSortCol.name !== this.sortColumn || defaultSortCol.defaultSort !== this.sortDirection) {
