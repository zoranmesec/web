...
import { Area, Country, IceFall, IceFallsCountryGQL, IceFallsCountryQuery } from '../../../../generated/graphql';
...
export class CountryComponent implements OnInit {
    ...
    queryError(errors?: readonly GraphQLError[]) {
        if (errors && errors.length > 0 && errors[0].message === 'entity_not_found') {
            this.error = {
    ...
    querySuccess(result: IceFallsCountryQuery) {
        this.areas = this.countIceFallsAndFlatten(result.countryAreas.areas as Area[]).filter((area) => area.nrIceFalls > 0);

        this.country = result.countryAreas as Country;
        this.countries = result.countries as Country[];
        this.iceFalls = result.countryIceFalls.iceFalls as IceFall[];
    ...
    changeArea(slug: string) {
        this.router.navigate(['/alpinizem/slapovi/drzava', this.country.slug, { obmocje: slug }]);
    }
    ...
    private countIceFallsInBranchAndFlatten(area: Area, depth = 0): FlatArea[] {
        ...
        area.areas.forEach((area) => {
            const childAreaWICnt = this.countIceFallsInBranchAndFlatten(area, depth + 1);
