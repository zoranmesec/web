...
echarts.use([BarChart, GridComponent, CanvasRenderer, TooltipComponent, LegendComponent, ToolboxComponent, LineChart, TitleComponent]);
@Component({
...
export class ActivityStatisticsComponent implements OnInit, OnDestroy {
    ...
    activityTypes = ACTIVITY_TYPES.filter((a) => a.value !== 'peak' && a.value !== 'iceFall');
    ...
    ngOnInit(): void {
        ...
        this.subscription = this.authService.currentUser
            ...
            .subscribe({
                next: async (result) => {
                    this.loading = false;
                    this.myRouteStats = result.data.myRoutesStatistics as StatsRoutes[];
        ...
        this.subscriptionActivity = this.authService.currentUser
            ...
            .subscribe({
                next: async (result) => {
                    this.loading = false;
                    this.myActivityStats = result.data.myActivitiesStatistics as StatsActivities[];
    ...
    calcRoutesByYear() {
        for (const element of this.myRouteStats[Symbol.iterator]()) {
    ...
    async querySuccess(year) {
        ...
        const tempData = [];
        this.data = [];
        for await (const element of this.myRouteStats[Symbol.iterator]()) {
            if (!(year && element.year !== year)) {
                try {
                    const grade = await this.GradingSystemsService.diffToGrade(element.difficulty, 'french', false);
        ...
        const sort = Object.keys(tempData).sort().reverse();
        this.xAxisData = sort;
    ...
    buildOptions() {
        const series = this.selectedAscentTypes.map((x) => {
            const sums = [];
    ...
    buildOptionsActivitiesByYear() {
        ...
        const series = this.activityTypes.map((x) => {
            const sums = [];
