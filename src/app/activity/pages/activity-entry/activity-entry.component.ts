...
export class ActivityEntryComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        const typeSub = this.activatedRoute.data.subscribe(({ type }) => (this.type = type));
        ...
        const routeSub = this.activatedRoute.params
            ...
            .subscribe({
                next: (result) => {
                    this.loading = false;
                    this.activity = result.data.activity as ActivityEntryQuery['activity'];

                    this.activityType = ACTIVITY_TYPES.find((t) => t.value === result.data.activity.type);

                    this.title = dayjs(this.activity.date).format('D. M. YYYY') + ' - ' + (this.activity.name || this.activityType.label);
