...
import { concatMap, debounceTime, filter, Subject, Subscription, switchMap, take } from 'rxjs';
...
import { SearchComponent, SearchType } from 'src/app/pages/search/search.component';
...
export class ActivityRoutesComponent implements OnInit, OnDestroy {
    ...
    gradeForm = this.fb.group({
        minGrade: this.fb.control({ value: MIN_GRADE, disabled: false }, { validators: [], nonNullable: true }),
        maxGrade: this.fb.control({ value: MAX_GRADE, disabled: false }, { validators: [], nonNullable: true })
    });
    ...
    protected gradingSystemId = 'french';
        ...
        this.subscriptions.push(
            this.columnForm.valueChanges.subscribe((value) => {
                ...
                for (const key of Object.keys(value)) {
                    if (value[key]) {
                        selectedColumns[key] = this.tableColumns.find((column) => column.name === key);
                    }
                }

                localStorage.setItem('activityRoutesSelectedColumns', JSON.stringify(Object.keys(selectedColumns)));
            })
        );
    }
    ...
    ngOnInit(): void {
        ...
        this.publishOptions.forEach((option) => {
            (this.filters.get('publish') as FormGroup).addControl(option.value, new FormControl(false));
        });
            ...
            .pipe(
                switchMap((params) => {
                    ...
                    try {
                        ...
                        console.error('Error setting route params:', error);
                    }
                    this.ignoreFormChange = true;
                    this.filters.patchValue(this.filteredTable.filterParams, { emitEvent: false });
                    if (this.filteredTable.filterParams.minGrade !== null) {
                        this.gradeForm.patchValue({ minGrade: this.filteredTable.filterParams.minGrade }, { emitEvent: false });
                    } else {
                        this.gradeForm.patchValue({ minGrade: MIN_GRADE }, { emitEvent: false });
                    }
                    if (this.filteredTable.filterParams.maxGrade !== null) {
                        this.gradeForm.patchValue({ maxGrade: this.filteredTable.filterParams.maxGrade }, { emitEvent: false });
                    } else {
                        this.gradeForm.patchValue({ maxGrade: MAX_GRADE }, { emitEvent: false });
                    }
                    this.applyRelationFilterDisplayValues();

                    this.loading = true;

                    const queryParams: FindActivityRoutesInput = this.filteredTable.queryParams;
            ...
            .subscribe({
                next: (result) => {
                    if (result.data !== undefined) {
                        this.loading = false;
                        this.filteredTable.navigating = false;
                        this.ignoreFormChange = false;
                        this.querySuccess(result.data.myActivityRoutes as MyActivityRoutesQuery['myActivityRoutes']);
                    }
                },
                error: () => {
                    this.queryError();
        ...
        const filtersSub = this.filters.valueChanges
            ...
            .subscribe((values) => {
                if (ft.navigating) {
                    ...
                    newParams.publish = Object.keys(newParams.publish).filter((key) => newParams.publish[key] === true);
        ...
        const topRopeSub = this.filters.get('topRope').valueChanges.subscribe(() => {
    ...
    formatLabel(value: number, isMin = false): string {
        let myValue = value;
    ...
    get nrOfFiltersApplied(): number {
        ...
        if (filterValues.publish['club'] || filterValues.publish['private'] || filterValues.publish['public']) count++;
        if (filterValues.sport || filterValues.boulder || filterValues.multipitch) count++;

        const gradeValues = this.gradeForm.value;
        if ((gradeValues.minGrade && gradeValues.minGrade !== MIN_GRADE) || (gradeValues.maxGrade && gradeValues.maxGrade !== MAX_GRADE))
            count++;
        return count;
    }
    ...
    querySuccess(data: MyActivityRoutesQuery['myActivityRoutes']): void {
        ...
        this.noTopropeOnPage = !this.routes.some((route) => {
            const index = this.ascentTypes.find((at) => at.value === route.ascentType);
            if (index) {
