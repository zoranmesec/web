...
export class ActivityFormRouteComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.route.controls.publish.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
    ...
    conditionallyRequireVotedDifficulty(ascentTypeSelected: string) {
        const isTick = ASCENT_TYPES.some((at) => at.value === ascentTypeSelected && at.tick);
        if (isTick) {
            ...
            this.route.setValidators((formGroup) => Validators.required(formGroup.get('votedDifficulty')));
        } else {
    ...
    logPossibleEver(ascentType: string) {
        if (this.route.value.type === 'boulder' && ['onsight', 't_onsight'].includes(ascentType)) {
            return false;
        }
        return true;
    }
