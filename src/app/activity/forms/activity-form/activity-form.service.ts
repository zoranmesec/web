...
export class ActivityFormService {
    ...
    tickAscentTypes = new Set(ASCENT_TYPES.filter((at) => at.tick).map((at) => at.value));

    trTickAscentTypes = new Set(ASCENT_TYPES.filter((at) => at.topRopeTick).map((at) => at.value));
    ...
    starRatingVotesForRoutes: Record<string, number>; // This is an object of routeId=>stars pairs, that is user's possible previous star rating vote on each route.

    initialize(routes: FormArray) {
        this.routesBeingLoggedFormArray = routes;
        this.distinctRouteIds = new Set(routes.controls.map((routeFormGroup) => routeFormGroup.get('routeId').value));
        this.routesPossibleAscentTypes = new Array(this.numOfRoutes).fill(new Set());
    }
    ...
    revalidateAscentTypes() {
        this.routesBeingLoggedFormArray.controls.forEach((routeFormGroup, currRouteIdx) => {
            const routeId = routeFormGroup.get('routeId').value;

            // Start with all possible ascentTypes
            this.routesPossibleAscentTypes[currRouteIdx] = new Set(this.allPossibleAscentTypes);
            ...
            for (let prevRouteIdx = 0; prevRouteIdx < currRouteIdx; prevRouteIdx++) {
                if (routeId !== this.routesBeingLoggedFormArray.controls[prevRouteIdx].get('routeId').value) {
                    continue; // This is not another instance of the same route, so skip it.
                }

                const previousAscentType = this.routesBeingLoggedFormArray.controls[prevRouteIdx].get('ascentType').value;
            ...
            if (currentAscentType && !this.routesPossibleAscentTypes[currRouteIdx].has(currentAscentType)) {
    ...
    duplicateRoute(routeIndex: number) {
        const routeFormGroupOriginal = this.routesBeingLoggedFormArray.at(routeIndex) as UntypedFormGroup;
    ...
    moveRoute(routeIndex: number, direction: number) {
        ...
        [this.routesBeingLoggedFormArray.controls[routeIndex], this.routesBeingLoggedFormArray.controls[routeIndex + direction]] = [
            this.routesBeingLoggedFormArray.controls[routeIndex + direction],
            this.routesBeingLoggedFormArray.controls[routeIndex]
        ];

        [this.routesPossibleAscentTypes[routeIndex], this.routesPossibleAscentTypes[routeIndex + direction]] = [
    ...
    private copyFormGroup(formGroupOriginal: UntypedFormGroup) {
        const formGroupData = Object.keys(formGroupOriginal.controls).reduce((fgData, key) => {
            fgData[key] = new UntypedFormControl(formGroupOriginal.get(key).value, formGroupOriginal.get(key).validator);
            return fgData;
        }, {});
    ...
    conditionallyDisableVotedDifficultyInputs() {
        this.distinctRouteIds.forEach((routeId) => {
            ...
            this.routesBeingLoggedFormArray.controls
                .filter((routeFormGroup) => routeFormGroup.get('routeId').value === routeId)
                .forEach((routeFormGroup) => {
                    ...
                    if (this.tickAscentTypes.has(routeFormGroup.get('ascentType').value) && !someVDIEnabled) {
                        someVDIEnabled = true;
                        ...
                        routeFormGroup.get('votedDifficulty').setValue(null, { emitEvent: false });
            ...
            this.routesBeingLoggedFormArray.controls
                ...
                .forEach((routeFormGroup) => {
                    if (!someVSRIEnabled) {
                        ...
                        routeFormGroup.get('votedStarRating').setValue(null, { emitEvent: false });
