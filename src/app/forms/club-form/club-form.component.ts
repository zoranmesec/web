...
import { CreateClubGQL, UpdateClubGQL } from 'src/generated/graphql';

@Component({
    selector: 'app-club-form',
    templateUrl: './club-form.component.html',
    styleUrls: ['./club-form.component.scss'],
    imports: [MatFormField, MatLabel, MatDialogActions, MatDialogModule, FormsModule, ReactiveFormsModule]
})
export class ClubFormComponent implements OnInit {
    title: string;
    clubForm = new UntypedFormGroup({
    ...
    updateClub() {
        ...
        this.updateClubGQL.mutate({ variables: { input: updateClubInput } }).subscribe((result) => {
            if (result.error !== null) {
                this.queryError(result.error);
            } else {
                this.router.navigate(['/moj-profil/moji-klubi', result.data.updateClub.slug]);
    ...
    createClub() {
        ...
        this.createClubGQL.mutate({ variables: { input: { name } } }).subscribe((result) => {
            if (result.error !== null) {
                this.queryError(result.error);
            } else {
    ...
    queryError(errors: ErrorLike) {
        if (errors.message.startsWith('Could not find any entity of type')) {
            ...
            this.displayError('Samo administratorji kluba lahko spremenijo ime kluba.');
