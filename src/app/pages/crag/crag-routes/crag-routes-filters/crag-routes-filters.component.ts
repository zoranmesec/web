...
export class CragRoutesFiltersComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.filtersForm.controls['minDifficulty'].valueChanges.subscribe(
      (value) => {
        cragRoutesFiltersService.setMinGrade(value);
      }
    );

    this.filtersForm.controls['maxDifficulty'].valueChanges.subscribe(
      (value) => {
