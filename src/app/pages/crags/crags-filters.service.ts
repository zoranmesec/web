...
export class CragsFiltersService {
    ...
    toggleRouteType(routeType: string) {
        this.routeTypes.update((current) =>
            current.includes(routeType) ? current.filter((type) => type !== routeType) : [...current, routeType]
        );
    }

    toggleArea(area: string) {
        this.areas.update((current) => (current.includes(area) ? current.filter((a) => a !== area) : [...current, area]));
    }
}
