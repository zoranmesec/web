...
export class ClubComponent implements OnInit, OnDestroy {
    ...
    queryError(error: Error) {
        if (error?.message.startsWith('Could not find any entity of type')) {
            ...
            this.error = {
                message: 'Nisi član kluba, zato nimaš pravic za prikaz podatkov o klubu.'
            };
        } else {
            this.error = {
    ...
    delete() {
        this.dialog
            .open(ConfirmationDialogComponent, {
                data: {
                    message: 'Ali res želiš odstraniti vse člane iz kluba in izbrisati klub?'
                }
            })
            .afterClosed()
            .pipe(
    ...
    displaySuccess() {
        this.snackbar.open('Vsi člani so bili odstranjen iz kluba in klub je bil uspešno izbrisan.', null, {
            duration: 3000
        });
    }
