...
export class ClubActivityRoutesComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.ftNavSubscription = this.filteredTable.navigate$.subscribe((ftParams) => {
            return this.router.navigate(['/moj-profil/moji-klubi', clubSlug, 'vzponi', ftParams]);
        });
    ...
    querySuccess(data: ActivityRoutesByClubSlugQuery) {
        ...
        this.noTopropeOnPage = !this.activityRoutes.some((route) => this.ascentTypes.find((at) => at.value === route.ascentType).topRope);
