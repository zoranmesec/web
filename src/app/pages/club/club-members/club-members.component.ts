...
import { Club, DeleteClubMemberGQL, namedOperations } from 'src/generated/graphql';
...
export class ClubMembersComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.subscription = this.authService.currentUser.subscribe((user) => (this.meId = user ? user.id : null));
    }
    ...
    queryError(error?: ErrorLike) {
        ...
        if (error && error.message === 'Forbidden') errorMessage = 'Nimaš pravic za odstranjevanje članov.';
