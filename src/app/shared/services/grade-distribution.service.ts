...
export class GradeDistributionService {
    ...
    async getDistribution(difficultyVotes: { difficulty: number }[], gradingSystemId: string): Promise<IDistribution[]> {
        ...
        return new Promise((resolve) => {
            ...
            Object.keys(difficultyCounts).forEach((grade: string): void => {
                diffToGradePromises.push(this.gradingSystemsService.diffToGrade(Number(grade), gradingSystemId, true));
            });

            Promise.all(diffToGradePromises).then((distributions) => {
                const gradesDistribution: IDistribution[] = distributions.map((distribution, i) => ({
