...
export class GradingSystemsService {
    ...
    async getGradingSystems(): Promise<GradingSystemsQuery['gradingSystems']> {
    ...
    diffToGrade(difficulty: number, gradingSystemId: string, legacy = false): Promise<IGrade> {
                    ...
                    if (legacy && gradingSystemId === 'french') {
                        for (let i = 1; i < grades.length; i++) {
                            ...
                            if ((prev.difficulty + curr.difficulty) / 2 === difficulty) {
                                return resolve({
                                    name: this.parseLegacyFrenchInBetweenName(prev.name, curr.name),
                                    modifier: 0,
