...
interface Area {
    ...
    name: string;
    slug: string;
    peaks: { __typename?: 'Peak'; name: string }[];
...
export class PeaksCountryComponent implements OnInit, OnDestroy {
    ...
    changeArea(areaSlug: string) {
        this.router.navigate(['/alpinizem/vrhovi/drzava', this.countrySlug, areaSlug ? { obmocje: areaSlug } : {}]);
    }
    ...
    private countPeaksInBranchAndFlatten(area: Area, depth = 0) {
